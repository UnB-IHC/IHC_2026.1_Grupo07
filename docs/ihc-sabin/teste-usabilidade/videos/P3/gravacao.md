# Gravação - Participante 3

Abaixo está o registro em vídeo do Teste de Usabilidade realizado com o Participante 3. Diferente das sessões de P1 e P2 — que cobriram apenas a Tarefa 1 —, nesta sessão de aproximadamente 10 minutos foram aplicadas as **três tarefas** do roteiro. O vídeo captura a tela do usuário bem como suas verbalizações utilizando a técnica de *Think Aloud*.

**Link para o vídeo:**
[Assistir à gravação no YouTube](https://www.youtube.com/watch?v=Yy9u99nBeJw)

## Resumo da Tarefa

**Tarefas aplicadas** (na ordem em que foram executadas no vídeo):

1. Agende um exame de hemograma completo na unidade Ceilândia Centro. *(T1 do roteiro)*
2. Veja as orientações de preparo para um check-up executivo. *(T3 do roteiro)*
3. Agende uma vacina de febre amarela para um dependente de 9 meses na unidade Águas Claras. *(T2 do roteiro)*

**Participante:** Estudante de Engenharia de Software, sexo feminino (conforme critério de recrutamento). Demais dados demográficos não foram verbalizados na sessão.

**Resultado:** A participante concluiu as três tarefas, porém com experiências muito diferentes. O **agendamento de exame (T1)** e o **agendamento de vacina (T2)** foram difíceis: ela se perdeu na navegação e só concluiu **com dicas do avaliador**, estranhando que fosse preciso usar "Compra online" para agendar e que "Agendar exame" levasse ao atendimento móvel. Já o **preparo do check-up (T3)** foi rápido e intuitivo, concluído sem ajuda. No feedback final, resumiu: *"não imaginaria que fazer uma compra online me levaria a agendar uma vacina pra criança"*.

> A transcrição e a análise detalhada (citações, caminho percorrido e SUS estimado) estão consolidadas em [Resultados — seção 4 (Entrevista 3)](../../resultados.md#4-resultado-entrevista-3-p3).
