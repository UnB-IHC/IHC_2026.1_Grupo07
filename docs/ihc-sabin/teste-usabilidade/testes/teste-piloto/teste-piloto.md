...
-   <span class="badge" style="background:#2e7d32;color:#fff;">Sinal verde</span> para as sessões oficiais
...
```mermaid
flowchart LR
    A[Apresentação] --> B[Assinatura do TCLE]
    B --> C[Explicação do objetivo]
    C --> D[Execução da tarefa]
    D --> E[Feedback qualitativo]
    E --> F[Questionário SUS]
    F --> G[Finalização]
